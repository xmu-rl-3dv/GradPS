...
def run_sequential(args, logger):
    ...
    while runner.t_env <= args.t_max:
        ...
        if (runner.t_env - last_test_T) / args.test_interval >= 1.0:
            ...
            #     learner.masker()
            #     for _ in range(n_test_runs):
            #         runner.run(test_mode=True, mask_mode=True)
            #     learner.masker()
            ...
            if "_prun" in args.name:
                prun_time = prun_time + 1
                if prun_time == T:
                    learner.prun()
                    prun_time = 0

            if "_inject" in args.name:
                inject_time = inject_time + 1
                if inject_time == args.t_max//100000*5:
                    print('inject',runner.t_env)
                    learner.inject()
                    inject_time = 0
            ...
            if "_r" in args.name or "_u" in args.name:
                # if mix_delay_time>0:
                #     learner.recycle()
                #     mix_delay_time-=1
                #     if mix_delay_time==0:
                #         learner.unfixed()
                #         learner.last_target_update_episode = 0
                recycle_time = recycle_time + 1
                if recycle_time == T:
                    learner.recycle()
                    recycle_time = 0
