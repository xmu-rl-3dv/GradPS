...
class StagHunt(MultiAgentEnv):
    ...
    def __init__(self, batch_size=None, **kwargs):
        ...
        self.n_hunts = args.n_hunts
        self.hunts = np.random.permutation(self.n_agents)[:self.n_hunts]
    ...
    def step(self, actions):
        ...
        if not self.batch_mode:
            actions = np.expand_dims(np.asarray(actions.cpu(), dtype=int_type), axis=1) #change from actions to actions.cpu(), 20211201
            ...
            for p in np.random.permutation(self.n_prey):
                if self.prey_alive[p, b] > 0:
                    ...
                    if captured:
                        ...
                        if double_reward and self.prey_type[p, b] == 2:
                            rew = rew * 5
